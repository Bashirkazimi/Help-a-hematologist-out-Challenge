...
from data import create_dataset, get_source_and_target
...
if __name__ == '__main__':
    ...
    dataset, val = get_source_and_target('metadata.csv', batch_size=opt.batch_size, num_workers=opt.num_threads)
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        os.makedirs('figures/epoch_{}'.format(epoch), exist_ok=True)
        save_it(visuals, 'figures/epoch_{}'.format(epoch))
